...
st.markdown("""
Welcome to the first website where one can directly estimate the monthly photovoltaic (PV) generation lost due to snow using Townsend’s method. Snow on PV will significantly reduce its output, but by how much? This tool calculates that amount. The main influences are the quantity of snow and the tilt angle of the PV array. The calculation also relies on a few additional weather and system geometry inputs, with guidance on the left side of the page. Townsend’s snow loss equations were developed based on two winters of field measurements done near Lake Tahoe, California from 2009 to 2011. The equation and the losses measured for several tilt angles were published in 2011 at the 37th IEEE Photovoltaic Specialists Conference in Seattle, Washington.
""")
...
st.markdown("""
Prior to this webpage, users needed to create an Excel worksheet, obtain a courtesy worksheet from the author, or, since 2023, execute the model through a Python-based implementation that was added to the pvlib platform in 2023. Since its publication in 2011, the equation has been amended slightly to account for a broader amount of PV installation types. For example, bifacial PV is now a major commercial market. Compared to traditional monofacial PV, it will experience less snow loss, owing to its more absorptive and snow-free back side. This webpage allows the user to click a bifacial option. The resulting annual snow loss will typically decrease by about a percent or so. Two other features implemented here include guidance for modeling tracking PV, as well as an option to reduce the estimated snow loss by 25% in instances where multiple paralleled DC source circuits exist in the upslope direction. For example, if modules are oriented in landscape, bypass diodes will gradually restore generation as snow slides down and uncovers the upper portions of a module.

A link to a similarly looking and functioning webpage is provided below. This option draws directly from the pvlib code and does not allow the bifacial option to be run. Its dust loss component uses a sequence provided by Kajal Sheth, the author of both webpages. Results from either version are likely to be close and functionally nearly identical.

**A key usage tip:** 15 years of experience have shown that reporting the amount of monthly snowfall should be done with careful attention to the snowfall measurement method. While not the only way to measure snow, once per day snow depth readings are prone to under-reporting the amount of true snowfall. Feedback suggests the Townsend model tends to be optimistic relative to that measured in the field or predicted by the other prominent snow model from NREL. As snow settles, compresses, melts, or evaporates, infrequent snow depth readings will suggest less snowfall has occurred than readings taken more frequently, for example, once per hour. Once per day readings are probably the most common method of gauging snowfall. However, the author has observed differences approaching 50% between daily snow depth data and the more accurate snowfall recorded via hourly observation or other methods such as sonar or water-equivalent heated sensors. The provisional guidance offered here is to **consider increasing any** ***daily*** **snow depth-based records by 25%**. This will provide results that are in better accordance with the original snowfall records used to create the correlation.

Lastly, this webpage introduces a companion dust loss model, also developed by Townsend. It relies mostly on a series of monthly precipitation inputs. It allows the user to specify whether up to two manual washes per year are to be done and, if so, optimizes when these cleanings should be done. It also allows the user to specify a seasonally appropriate daily dust build-up rate, with a nominally safe default ramp-up of 0.1% per day. The theory behind the Townsend dust model is available via a PDF link below, as is the theory associated with the snow model.

While the snow and dust models can be run here separately, the main use of either is to provide monthly soiling loss inputs for PVsyst or similar simulation programs. Therefore, it is anticipated that users will benefit from running both models in order to obtain a complete year of monthly inputs for PVsyst. For months in which there may be non-zero losses calculated for both snow and dust, the overlap logic for this webpage’s combined loss tables and graphic assumes a snow loss of 3% or more in any month will render that month’s dust loss zero. For transitional months with snow loss less than 3%, the dual losses are combined using a simple overlap equation: Loss = A + B - (A * B), where A and B represent the monthly fractional snow and dust loss.
""")
