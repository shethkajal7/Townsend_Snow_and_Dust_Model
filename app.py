...
**A key usage tip:** 15 years of experience have shown that reporting the amount of monthly snowfall should be done with careful attention to the snowfall measurement method. While not the only way to measure snow, periodic snow depth readings are prone to under-reporting the amount of true snowfall, and internet feedback suggests the Townsend model tends to be optimistic relative to that measured in the field or predicted by the other prominent snow model from NREL. As snow settles, compresses, melts, or evaporates, infrequent snow depth readings will suggest less snowfall has occurred than readings taken more frequently, for example, once per hour. Once per day readings are probably the most common method of gauging snowfall. However, the author has observed differences approaching 50% between daily snow depth data and the more accurate snowfall recorded via hourly observation or other methods such as sonar or water-equivalent heated sensors. The provisional guidance offered here is to **consider increasing any** ***daily*** **snow depth-based records by 25%**. This will provide results that are in better accordance with the original snowfall records used to create the correlation.
