...
if run:
    ...
    if events_have_ge1.startswith("YES"):
        ...
        snow_events_any = [None if v is None or (isinstance(v, float) and pd.isna(v)) else float(v) for v in vals]
    ...
    dust_profile_data = pd.DataFrame({
        "Period": MONTHS + ["Ann."],
    ...
    dust_profile_data["Period"] = pd.Categorical(
        dust_profile_data["Period"],
        categories=MONTHS + ["Ann."],
        ordered=True,
    )
    dust_profile_plot = dust_profile_data.melt(
        id_vars="Period",
    ...
    st.altair_chart(dust_profile_chart, use_container_width=True)
